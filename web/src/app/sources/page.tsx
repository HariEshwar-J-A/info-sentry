...
function AddSourceForm({ onAdded }: { onAdded: (s: Source) => void }) {
  ...
  if (!open) return (
    <button onClick={() => setOpen(true)} style={{ width: '100%', padding: '12px', borderRadius: '10px', border: '1px dashed #2a2a2a', background: 'none', color: '#6366f1', cursor: 'pointer', fontSize: '13px', fontWeight: 600 }}>
      + Add Global Source
    </button>
  )
  ...
  return (
    <form onSubmit={e => void handleAdd(e)} style={{ backgroundColor: '#111', border: '1px solid #2a2a2a', borderRadius: '12px', padding: '16px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
      ...
      <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
        <input value={name} onChange={e => setName(e.target.value)} placeholder="Name (e.g. TechCrunch)" required
          style={{ flex: '1 1 160px', background: '#1a1a1a', border: '1px solid #2a2a2a', borderRadius: '6px', color: '#e0e0e0', fontSize: '13px', padding: '8px 12px', outline: 'none' }} />
        <input value={url} onChange={e => setUrl(e.target.value)} placeholder="URL" required
          style={{ flex: '2 1 240px', background: '#1a1a1a', border: '1px solid #2a2a2a', borderRadius: '6px', color: '#e0e0e0', fontSize: '13px', padding: '8px 12px', outline: 'none' }} />
      </div>
      <input value={rssUrl} onChange={e => setRssUrl(e.target.value)} placeholder="RSS feed URL (optional — strongly recommended)"
        style={{ background: '#1a1a1a', border: '1px solid #2a2a2a', borderRadius: '6px', color: '#e0e0e0', fontSize: '13px', padding: '8px 12px', outline: 'none' }} />
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
      ...
      <div style={{ display: 'flex', gap: '8px' }}>
        <button type="submit" disabled={loading || !url.trim() || !name.trim()}
          style={{ padding: '7px 18px', borderRadius: '7px', border: 'none', background: loading ? '#1a1a1a' : 'linear-gradient(135deg,#6366f1,#8b5cf6)', color: loading ? '#555' : '#fff', cursor: 'pointer', fontSize: '13px', fontWeight: 600 }}>
          {loading ? 'Adding…' : 'Add Source'}
        </button>
        <button type="button" onClick={() => setOpen(false)} style={{ padding: '7px 12px', borderRadius: '7px', border: '1px solid #2a2a2a', background: 'none', color: '#8a8a8a', cursor: 'pointer', fontSize: '13px' }}>
          Cancel
        </button>
      </div>
    </form>
      ...
      <div style={{ padding: '24px 32px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center' }}>
          <input
            ...
            style={{ flex: '1 1 200px', background: '#111', border: '1px solid #2a2a2a', borderRadius: '8px', color: '#f0f0f0', fontSize: '13px', padding: '8px 14px', outline: 'none' }}
          />
          <div style={{ display: 'flex', gap: '4px', backgroundColor: '#0d0d0d', border: '1px solid #1f1f1f', borderRadius: '8px', padding: '3px' }}>
            {FILTERS.map(f => (
              <button key={f.key} onClick={() => setFilter(f.key)}
                style={{ padding: '5px 12px', borderRadius: '5px', border: 'none', cursor: 'pointer', fontSize: '12px', fontWeight: 500, background: filter === f.key ? '#1a1a1a' : 'none', color: filter === f.key ? '#f0f0f0' : '#555' }}>
                {f.label}
              </button>
            ))}
          </div>
        ...
        {loading ? (
          <div style={{ color: '#555', fontSize: '14px', padding: '40px 0' }}>Loading sources…</div>
        ) : (
          <>
            {/* Active sources */}
            <div>
