      ...
      <div className="page-content">
        ...
        <div style={{ marginBottom: '24px' }}>
          <SegmentTabs
            variant="tabs"
            active={tab}
            onChange={id => id && setTab(id)}
            items={TABS.map(t => ({ id: t.id, label: t.label, count: t.badge > 0 ? t.badge : undefined }))}
          />
        </div>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '16px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                <SegmentTabs
                  variant="pills"
                  active={allStatusFilter}
                  onChange={id => id && setAllStatusFilter(id)}
                  items={[
                    { id: 'all', label: 'All' },
                    { id: 'PENDING', label: 'Pending' },
                    { id: 'CORRECT', label: 'Correct' },
                    { id: 'INCORRECT', label: 'Incorrect' },
                    { id: 'PARTIALLY_CORRECT', label: 'Partial' },
                  ]}
                />
            ...
            {filteredAll.length === 0 ? (
              <EmptyState
                title={all.length === 0 ? 'No predictions yet' : 'No predictions match your filter'}
                description={all.length === 0 ? 'Run the pipeline to generate some' : 'Try a different search term or filter'}
              />
            ) : (
                ...
                {filteredAll.map(pred => {
                  ...
                  return (
                    <div key={pred.id} style={{ backgroundColor: '#111', border: `1px solid ${statusColor[pred.status] ? statusColor[pred.status] + '33' : '#1f1f1f'}`, borderRadius: '12px', overflow: 'hidden' }}>
                      <div style={{ padding: '14px 16px', cursor: 'pointer', display: 'flex', alignItems: 'flex-start', gap: '12px' }} onClick={() => setExpandedId(isOpen ? null : pred.id)} role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') setExpandedId(isOpen ? null : pred.id) }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap' }}>
              <SegmentTabs
                variant="pills"
                active={statusFilter}
                onChange={id => id && setStatusFilter(id)}
                items={[
                  { id: 'all', label: 'All' },
                  { id: 'PENDING', label: 'Pending' },
              ...
              <button onClick={() => setShowUnreadOnly(!showUnreadOnly)}
                style={{ padding: '5px 12px', borderRadius: '20px', border: `1px solid ${showUnreadOnly ? '#6366f1' : '#2a2a2a'}`, background: showUnreadOnly ? 'rgba(99,102,241,0.12)' : 'none', color: showUnreadOnly ? '#a5b4fc' : '#8a8a8a', cursor: 'pointer', fontSize: '12px', fontWeight: 500, transition: 'all 0.15s', fontFamily: 'inherit' }}>
            ...
            {filteredTracked.length === 0 ? (
              <EmptyState
                title={tracked.length === 0 ? 'No tracked predictions yet' : 'No predictions match your filters'}
                description={tracked.length === 0 ? 'Track predictions from article pages' : undefined}
              />
            ) : (
