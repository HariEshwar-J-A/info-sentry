...
import { logAlert } from './lib/telegram.js'
...
function runAgent(name: string, scriptRelPath: string, args: string[] = []) {
  ...
  child.on('close', (code) => {
    ...
    const errMsg = code !== 0 ? `Exit code ${code}` : null
    ...
    db.agentConfig.upsert({
      ...
      create: { agentName: name, lastRunAt: new Date(), lastError: errMsg },
    }).catch(() => {})

    // Alert on non-zero exit (agent crash)
    if (code !== 0) {
      logAlert('error', `${name} crashed`, { exitCode: code }).catch(() => {})
    }
  })
